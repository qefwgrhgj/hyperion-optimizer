...
public class HyperionFabric1211 implements ModInitializer {
    @Override
    public void onInitialize() {
        HyperionConfig config = HyperionConfigStorage.loadOrCreate();
        HyperionEngine.getInstance().initialize(config);
        System.out.println("[Hyperion] Initialized Hyperion Multi-Core Engine on Fabric 1.21 / 1.21.1");
    }

    public void onInitializeClient() {
        HyperionConfig config = HyperionConfigStorage.loadOrCreate();
        HyperionEngine.getInstance().initialize(config);
        System.out.println("[Hyperion] Initialized Hyperion Client & Keybindings on Fabric 1.21 / 1.21.1");
    }
}
