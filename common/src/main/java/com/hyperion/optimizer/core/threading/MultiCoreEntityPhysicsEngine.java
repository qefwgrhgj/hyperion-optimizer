...
public final class MultiCoreEntityPhysicsEngine {
    ...
    public void processEntityBatchParallel(List<EntityStateSnapshot> entities, EntityTickTask tickTask) {
        ...
        try {
            // Safety timeout: never hang main server loop
            boolean finished = latch.await(50, TimeUnit.MILLISECONDS);
            if (!finished) {
                // Main thread proceeds; remaining workers finish in background
            }
